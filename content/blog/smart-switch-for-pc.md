...
## What we need
  ...
  * An ESP32. I have an [ESP32-S3 "supermini"](https://www.aliexpress.com/item/1005010130447256.html) which is overkill for this, any ESP32 would work
  * A 5V relay or equivalent. Here I'm using a [relay module with a "SRD-5 VDC-SL-C" relay](https://www.aliexpress.com/item/1005009881252732.html). This is also probably 100x overkill and a transistor would work, but it's what I was familiar with.
...
When I first started this, I (having only the bare minimum electronics knowledge to get by) had hoped that I could implement this without any additional modules or components other than the dupont wires. I made a few mistakes like not realising that the front panel pins were 5V, then I realised I needed a relay, then I was trying with a relay that wasn't 5V, so it took me a few goes to land on a working setup.

Intuitively I thought that the motherboard's front panel ground power pin should be connected to "Normally Closed" (NC), and that "shorting" the connection would cause the PC to boot. Doing this just causes your PC to endlessly reboot. In reality, it should be connected to "Normally Open" (NO), and _opening_ the connection causes the PC to boot.

  * Motherboard front panel power button ground pin to relay's NO terminal
  ...
  - platform: gpio
    ...
    on_turn_on:
      then:
        ...
        - if:
            condition:
              - switch.is_on: my_power_button_output_switch  # just in case something goes wrong
            then:
              - lambda: !lambda 'ESP_LOGE("my_power_button_output_switch", "Relay was not turned off after 5s, turning off.");'
              - switch.turn_off: my_power_button_output_switch
